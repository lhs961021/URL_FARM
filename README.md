...
<pre>여러 사이트의 정보를 활용할 때 url의 정보가 많으면 한눈에 보기 힘들고 각 정보의 핵심을 파악하는것에 어려움을 느낄 수 있다. 
<pre>또한 전에 참고한 url을 다시 찾아볼 때 url만 보면 어떤 내용에 해당하는 것인지 모르고 저장해 둔 url이 많다면 찾고자 하는 url을 찾는 것이 어렵다. 
<pre>이러한 것에 대한 문제점을 파악하여 이를 개선하고자 한다. 
